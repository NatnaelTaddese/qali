...
export const enqueueConnectionBackfill = internalMutation({
  ...
  handler: async (ctx, args): Promise<null> => {
    ...
    for (const userId of new Set(page.userIds)) {
      ...
      await ctx.scheduler.runAfter(0, internal.migrations.backfillConnections.backfillUser, {
        userId,
        runId,
      });
    }
    ...
    if (next) {
      ...
      await ctx.scheduler.runAfter(
        0,
        internal.migrations.backfillConnections.backfillSharedRecords,
...
export const backfillUser = internalMutation({
  ...
  handler: async (ctx, args): Promise<null> => {
    ...
    await ctx.scheduler.runAfter(
      0,
      internal.migrations.backfillConnections.backfillUserRows,
      {
        userId: args.userId,
        connectionId,
...
export const backfillUserEvents = internalMutation({
  ...
  handler: async (ctx, args): Promise<null> => {
    ...
    if (!page.isDone) {
      await ctx.scheduler.runAfter(
        0,
        internal.migrations.backfillConnections.backfillUserEvents,
        { ...args, cursor: page.continueCursor },
      );
    } else {
      await ctx.scheduler.runAfter(
        0,
        internal.migrations.backfillConnections.backfillUserTail,
