// Bun-test-only guard; never deployed (the Convex bundler skips multi-dot
// basenames like *.test.ts). The directive marks the node:fs use as such.
"use node";
...
const ALLOWLIST = new Map<string, string>([
  ...
  [
    "convex/domains/sync/engine.itest.ts",
    "tests the drain-only applyEngagementScores path",
  ],
  [
    "convex/domains/calendar/dualWrite.itest.ts",
    "tests the drain-only upsertEvent/upsertRecurringSeries shims",
  ],
  [
    "convex/migrations/scheduledJobs.itest.ts",
    "schedules the legacy path on purpose to prove the migration",
  ],
  [
